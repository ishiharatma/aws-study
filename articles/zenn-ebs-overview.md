...
![blackbelt-ebs](/images/blackbelt/blackbelt-ebs-320.jpg)
...
- オンプレサーバの物理的なディスクに近いイメージを持つかもしれませんが、EBS はネットワーク接続型です。Windows でいうとネットワークドライブを割り当てているイメージが近いかもしれません。
...
IOPS が高いからといって、低レイテンシであるとは限らない点には注意が必要です。
...
EBS には大きく分けて **SSD タイプ**と **HDD タイプ**があります。
SSD タイプはランダム I/O 性能を重視するワークロード向け、HDD タイプはシーケンシャル I/O や大容量データ処理向けです。
...
- プロビジョンド IOPS SSD
  ...
  - io2 Block Express
    ...
    - スループット: 4,000 MiB/s
      - 0.256 MiB/s/IOPS なので、最大スループットを実現するには 16,000 IOPS が必要
    - マルチアタッチ: OK
  - io1
    - 耐久性: 99.8%～ 99.9%
    - ボリュームサイズ: 1 GiB から 16 TiB
    ...
    - スループット: 1,000 MiB/s
      - 最大スループットを実現するには 64,000 IOPS が必要（プロビジョンド IOPS あたり 16 KiB × 64,000 IOPS = 1,000 MiB/秒）
    - マルチアタッチ: OK
...
クラスタリングされたデータベースや High Availability シナリオに有用です。
...
スナップショットは EBS ボリュームの特定時点のバックアップで、S3 に保存されます。
増分バックアップ方式を採用しており、コスト効率よくデータを保護できます。
...
OS の破損や設定ミスが発生した場合に、EC2 インスタンスを停止せずにルートボリュームを以前の状態に戻せる機能です。
スナップショットからルートボリュームを復元するため、事前にスナップショットを取得しておく必要があります。
...
EBS のバックアップには、主に **Amazon Data Lifecycle Manager (DLM)** と **AWS Backup** の 2 つのサービスを利用できます。
EBS スナップショット専用のシンプルな自動化には DLM、複数サービスを横断して一元管理したい場合は AWS Backup が適しています。

### Amazon Data Lifecycle Manager (DLM)

[Amazon Data Lifecycle Manager](https://docs.aws.amazon.com/ja_jp/ebs/latest/userguide/snapshot-lifecycle.html)

EBS スナップショットの作成・保持・削除を自動化するサービスです。

- タグベースのポリシーで対象ボリュームを指定
- スケジュール設定で自動的にスナップショットを作成
- 保持期間の設定により古いスナップショットを自動削除
- クロスリージョンコピーや共有ポリシーもサポート

### AWS Backup

[AWS Backup](https://docs.aws.amazon.com/ja_jp/aws-backup/latest/devguide/whatisbackup.html)

複数の AWS サービスのバックアップを一元管理できるフルマネージドサービスです。

- EBS 以外にも RDS、DynamoDB、EFS、S3 など多数のサービスに対応
- バックアッププランを定義してスケジュールや保持期間を管理
- クロスリージョン・クロスアカウントのバックアップをサポート
- バックアップポリシーによるコンプライアンス対応と監査レポート機能

| 機能 | Amazon DLM | AWS Backup |
| --- | --- | --- |
| 主な対象 | EBS スナップショット | EBS・RDS・DynamoDB・EFS・S3 など |
| 設定の複雑さ | シンプル | 多機能 |
| クロスリージョン | ○ | ○ |
| クロスアカウント | ○ | ○ |
| コンプライアンス管理 | △ | ○ |
...
EBS ボリュームは KMS を使用して保存データを暗号化できます。暗号化はパフォーマンスへの影響がほぼなく、有効にすることを推奨します。
...
EBS 作成時に暗号化有無を指定できますが、AWSアカウント全体でデフォルトで暗号化するようにもできます。ただし、この設定は**リージョン固有の設定**となっていますので、**使用するリージョンごとに有効化する**必要があります。
...
作成済みのボリュームを直接暗号化することはできないため、スナップショットのコピーを経由した手順が必要です。
「デフォルト暗号化」を有効化する前に作成したボリュームや、セキュリティポリシーへの対応により暗号化が必要になった場合に必要な手順です。
...
暗号化の解除も直接は行えないため、レスキューインスタンスを使って手動でデータをコピーする手順が必要です。
暗号化した KMS キーが使えなくなった場合や、暗号化をサポートしないサービス・ツールへデータを移行する必要がある場合に必要な手順です。
...
EBS のパフォーマンスは、ボリュームタイプの選択だけでなく、インスタンスの設定やデータのレイアウトによっても左右されます。
アプリケーションの要件に合わせて以下のポイントを考慮することが重要です。
...
CloudWatch を使用して EBS のパフォーマンスやヘルスを監視できます。
I/O の遅延やキューの積み上がりを検知し、ボトルネックの特定や容量不足の予防に活用します。
...
Amazon EBS は、EC2 インスタンスにアタッチして使用する永続的なブロックストレージサービスです。
ワークロードに応じて SSD（gp3/io2）や HDD（st1/sc1）を使い分けることがコスト最適化のポイントです。
スナップショットによる増分バックアップ、KMS による暗号化、マルチアタッチなど、可用性・セキュリティ面での機能も充実しています。
パフォーマンス面では IOPS・スループット・レイテンシーの特性を理解した上で、EBS 最適化インスタンスや適切なボリュームタイプを選択することが重要です。
