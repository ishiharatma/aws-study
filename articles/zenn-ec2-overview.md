...
料金は秒単位で計算され、使わなかったコンピューティング時間には課金されません。最小課金時間は 60 秒で、60 秒未満で停止・終了した場合も 60 秒分が請求されます。
...
| G（GPU 搭載） | GPU 演算 | 機械学習トレーニング、グラフィックス処理 |
...
- 世代
  ...
  - すでに稼働しているインスタンスも、古い世代なら新しい世代にするだけでコスト削減できる場合がある
- オプション
  - 実装したいサービス次第だが、Graviton（g 系）を積極的に検討する
- インスタンスサイズ
  - 必要なスペックを選択する
  - 中規模程度であれば、「medium」や「large」で十分な場合もある
...
Amazon または検証済み Amazon パートナーが所有するパブリック AMI には [検証済みプロバイダー] のマークが付きます。
...
- 高速（NVMe SSD）
- インスタンスを停止・終了すると**データが消える**（一時的なストレージ）
...
EIP は、EC2 に割り当てられていなくても課金対象となりました。
:::
...
| 種類 | 特性 | 用途 |
| --- | --- | --- |
| クラスター | 単一のアベイラビリティーゾーン内のインスタンスを論理的にグループ化。低レイテンシ・高スループット | HPC、機械学習トレーニング |
...
- 秘密鍵は**一度しかダウンロードできません**。紛失した場合は再作成が必要です
...
| スポットインスタンス | AWS の余剰キャパシティを利用 | 最大 90% | バッチ処理、機械学習トレーニングなど中断を許容できるワークロード |
...
誤操作を防ぐために、本番インスタンスには終了保護（Termination Protection）を有効化することを推奨します。
:::
...
新規に EC2 を構築するときの出発点は、ほぼ決まっている。AMI は Amazon Linux 2023、EBS は gp3、接続は Session Manager。この 3 つを既定にすれば、SSH ポート（22 番）の開放もキーペアの受け渡しも不要になり、あとから見直す箇所が減る。インスタンスタイプは M（汎用）の最新世代を第一候補に、開発や小規模検証なら T 系のバースト、Linux ワークロードなら Graviton（g 系）を積極的に検討する。

コストを抑える軸は購入オプションと世代選択の 2 つ。スペックが長期間固定なら RI、インスタンスタイプを変える可能性があるなら Savings Plans、中断を許容できるバッチや機械学習トレーニングならスポットインスタンスが候補になる。世代は新しいほうが単価が低いことも多く、既存インスタンスを新世代に載せ替えるだけでコストが下がる場合がある。

運用とセキュリティでは、EC2 から AWS サービスを呼ぶときは必ず IAM ロールを使い、アクセスキーをインスタンスに埋め込まない。本番インスタンスは終了保護を有効化しておくと、誤操作による削除を防げる。インスタンスストアは停止でデータが消える一時領域なので、永続化が必要なものは EBS か S3 に置く。

課金の落とし穴はパブリック IPv4 アドレス。2024 年 2 月以降、EIP もパブリック IP も使用しているだけで時間課金され、EIP は EC2 に割り当てていない状態でも対象になる。使わない EIP は解放しておく。
