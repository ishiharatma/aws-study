---
title: "【初心者向け】AWS Amplify Gen2 入門！完全ガイド" # 記事のタイトル
...
:::message
**AWS CodeCommit について**
2024 年 7 月に新規サインアップが停止されましたが、2025 年 11 月 24 日に GA へ復活しました。新規顧客も含めて引き続き利用できます。

[The Future of AWS CodeCommit](https://aws.amazon.com/jp/blogs/devops/aws-codecommit-returns-to-general-availability/)
:::
...
- 2026 年 1 月の AWS Amplify Conference 2026 にて、Gen 1 から Gen 2 へのマイグレーションツールのベータ提供が発表されました
...
AWS Amplify Gen 2 は、TypeScript でバックエンドをコードとして定義できるフルスタック開発プラットフォームです。

- **Auth**（Cognito）、**Data**（AppSync + DynamoDB）、**Storage**（S3）、**Functions**（Lambda）が組み込みでサポート
- 開発者ごとのサンドボックス環境でチーム開発が快適になり、デプロイ速度は Gen 1 比で最大 8 倍高速
- CDK ベースのため、Amplify が標準提供しない AWS サービスも自由に追加できる
- Amplify Hosting は Next.js / Nuxt / Astro / SvelteKit をサポートし、Git ブランチと連動した CI/CD が利用できる

Gen 1 からの移行は必須ではありませんが、新規アプリケーションは Gen 2 が推奨です。
