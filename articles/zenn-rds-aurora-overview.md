---
title: "【初心者向け】Amazon Relational Database Service(RDS) と Aurora について改めて整理してみた" # 記事のタイトル
...
| 比較項目         | RDS                                        | Aurora                      |
|------------------|--------------------------------------------|-----------------------------|
| データベースエンジン       | MySQL,PostgreSQL,MongoDB,Oracle,SQL Server | MySQL,PostgreSQL            |
| ストレージアーキテクチャ     | EBSがインスタンス付属                             | Aurora クラスター全体で共有       |
| 耐久性           | インスタンス付属のミラーリング用EBSで複製                | 3AZ で複製                   |
| ストレージの自動スケーリング | 設定可                                     | デフォルトで自動拡張              |
| レプリカ             | 最大5台                                    | 最大15台                    |
| 可用性         | マルチAZのみ                                    | レプリカなしでもAZ障害以外は自動復旧 |
...
MySQL や PostgreSQL で、Aurora 制約事項（\*1、\*2）を許容できる場合は、Aurora が最適となるケースが多いです。
...
*3 例えば、RDS では Oracle RAC がサポートされていません。SQL Server では、sysadminなどのサーバーレベルロールが使用できません。詳細は、「よくある質問」やドキュメントを参照してください。
...
RDS ではリードレプリカを追加すると、「インスタンス + EBS + ミラーリング EBS 」のセットが増えていきます。
...
RDS は、0～35日(デフォルトは 7日)
RDS と Aurora のともに、1～35日(デフォルトは 1日)

RDS を停止した時間は保持期間の計算に含まれないので、保持期間より多いスナップショットが存在している場合もあります。
